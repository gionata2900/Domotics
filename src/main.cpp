...
#if defined(NDEBUG)
# error "Domotics cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5540713225e30d1004b9f3deb2f48c701c325794d3ceb2c02a9eda22cd2ebd1b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Domotics: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Domotics Signed Message:\n";
...
{
    // Domotics: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Domotics: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Domotics: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Domotics: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Domotics: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Domotics: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa55bca6a48077845e6856df2b87884088d27a6d5827bfad3186f8d4223a6e535");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "20.07.1969 primo uomo sulla luna";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04847094819760f4b31f3a6fa896451178dc30008a751cb5fc4c97f29f53202b699540b970e2e2b904aa0069cbf3e78b14b3f4c2967807ac787a65527be1d2c86d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519604367;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087431929;

        if (fTestNet)
        {
            block.nTime    = 1519604318;
            block.nNonce   = 385674078;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xac0ec880bae9a9a8fd23b70766b8e41ce9e9c82f1258debb94f0919139f5c8cb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc6, 0xb9, 0xd3 }; // Domotics: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DomoticsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DomoticsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DomoticsMiner(CWallet *pwallet)
{
    printf("DomoticsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Domotics-miner");
    ...
    try { loop {
        ...
        printf("Running DomoticsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DomoticsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DomoticsMiner, pwallet));
}
